...
export default function AdminFinanceiroPage() {
  ...
  async function carregarFinanceiro() {
    try {
      ...
      const [financeiroResponse, receitaResponse] = await Promise.all([
        fetch("/api/admin/financeiro", {
          cache: "no-store",
        }),
        fetch("/api/admin/financeiro/receita-mensal", {
          cache: "no-store",
        }),
      ]);
      ...
      if (!financeiroResponse.ok) {
        setErro(financeiroData.error || "Erro ao carregar financeiro");
        return;
      }

      if (!receitaResponse.ok) {
        setErro(receitaData.error || "Erro ao carregar gráfico de receita");
        return;
      }

      setDados(financeiroData);
      setReceitaMensal(Array.isArray(receitaData) ? receitaData : []);
    } catch (error) {
    ...
    <main className="min-h-screen bg-zinc-100 p-6 md:p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Valores confirmados registrados no histórico financeiro do SaaS.
          </p>

          <div className="mt-6">
            <RevenueChart data={receitaMensal} />
          </div>
        </section>
